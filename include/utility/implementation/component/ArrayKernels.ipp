		...
		namespace kernels
		{
			...
			I uniqueCount(T * source, I nEle)
			{
				...
				for(I i = 1; i < nEle; i++)
				{
					...
					{
						// Increase count of how many of current element we have encountered
						curCount = curCount + 1;
					}
					else
					{
			...
			void uniqueArray(T * source, T * dest, I nEle)
			{
				...
				for(I i = 1; i < nEle; i++)
				{
					if(source[i-1] == source[i])
					{
						curCount = curCount + 1;	// Increase count of current element
					}
					else
					{
						...
						if(curCount == 1)
						{
							...
							ptr = ptr + 1;				// Advance target copy location
						}
			...
			I intersectCount(T * source1, I nSource1, T * source2, I nSource2)
			{
				...
				for(I i = 0; i < sourceCap; i++)
				{
					...
					I index = cupcfd::utility::kernels::binarySearch(searchPtr, searchCap, sourcePtr[i]);
